...
3. *Link previews*. Are shown at the bottom of each post for each link in the post. Supports custom "Link Helpers" to generate different previews for different types of link. Shows the preview for the terminating link rather than the original link itself.
...
Link previews are generated by classes descending from the "LinkHelper" class in `inc/plugins/linktools/LinkHelperBase.php`, and stored under `inc/plugins/linktools/link-helpers/` in either of the `link-helpers-dist` or `link-helpers-3rd-party` subdirectories. You should not add to, delete from, or otherwise modify the contents of the first of those two subdirectories, because they may change on plugin upgrades, but you are free to add your custom Link Helpers into the second of those directories (just create it if it doesn't already exist). An explanation of how to write a Link Helper is beyond the scope of this README, but by looking at the existing Helpers, and especially the comments in their code, you will probably be able to figure it out.
